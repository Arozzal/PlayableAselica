...
{
	public class SwordRain : BaseSkillState
	{
		...
		public override void OnEnter()
		{
			...
			this.animator.Play("ThirdSkill", 0);

			SwordFalling.teamindex = teamComponent.teamIndex;
			SwordFalling.damageStat = this.damageStat;
			SwordFalling.damageCoefizienz = PlayableAselica.AselicaCastingTrail.damageCoefficient;
			...
			if (base.isAuthority)
			{
				if (!hasFired)
				{
					for (int i = 0; i < 200; i++)
					{
						...
						ProjectileManager.instance.FireProjectile(PlayableAselica.Assets.fallingSwordProjectile, this.characterBody.transform.position + new Vector3(x, y - 1.2f, z + 0.6f), this.gameObject.transform.rotation,
		...
		public void doDamage()
		{
			if (base.fixedAge >= this.fireDuration)
			{
				...
				var hurts = colls.Where(x => x.GetComponent<HurtBox>() != null && x.GetComponent<HurtBox>().teamIndex != base.teamComponent.teamIndex);
